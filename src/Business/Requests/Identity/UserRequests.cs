...
namespace Business.Requests.Identity
{
    ...
    public class CreateUserRequestHandler : IRequestHandler<CreateUserRequest, int>
    {
        ...
        public async Task<int> Handle(CreateUserRequest request, CancellationToken cancellationToken)
        {
            ...
            if (tba.Any())
            {
                await _userManager.AddToRolesAsync(entity, tba);
            }
        ...
        public CreateUserRequestValidator(IGenericRepository<User> repository)
        {
            ...
            RuleFor(m => new { m.Email })
                .CustomAsync(async (m, v, c) =>
                {
                    ...
                    if (isInUse)
                    {
                        v.AddFailure("Email is already in use");
                    }
                });
        }
    }
    ...
    public class UpdateUserRequestHandler : IRequestHandler<UpdateUserRequest>
    {
        ...
        {
            var entity = await _repository.FirstOrDefaultAsync(s => s, p => p.Id == request.Id, include: i => i.Include(m => m.UserClaims), cancellationToken: cancellationToken) ?? throw new NotFoundException(nameof(User), request.Id);
            ...
            if (entity.UserClaims == null || !entity.UserClaims.Any())
            {
            ...
            if (tbr.Any())
            {
                await _userManager.RemoveFromRolesAsync(entity, tbr);
            }
    ...
    public class PartialUpdateUserRequestHandler : IRequestHandler<PartialUpdateUserRequest>
    {
        ...
        public async Task Handle(PartialUpdateUserRequest request, CancellationToken cancellationToken)
        {
            ...
            {
                entity.EmailConfirmed = request.EmailConfirmed.Value;
            }
            ...
            if (request.PhoneNumberConfirmed != null)
            {
                entity.PhoneNumberConfirmed = request.PhoneNumberConfirmed.Value;
            }

            if (request.TwoFactorEnabled != null)
            {
                entity.TwoFactorEnabled = request.TwoFactorEnabled.Value;
            }
        ...
        public DeleteUserRequestValidator(IGenericRepository<User> repository)
        {
            ...
            RuleFor(m => new { m.Id })
                .CustomAsync(async (m, v, c) =>
                {
                    if (m.Id == 1)
                    {
                        v.AddFailure("Cannot delete the default user");
                    }

                    if (await repository.CountAsync(cancellationToken: c) == 1)
                    {
                        v.AddFailure("Cannot delete the last user");
                    }
                });
        }
    }
