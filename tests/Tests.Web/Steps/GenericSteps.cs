...
namespace Tests.Web.Steps
{
	...
	public class GenericSteps
	{
		...
		private int MaxAttempts => Program.Configuration?.GetValue<int>("Timeouts:MaxAttempts") ?? 1;

		private int Timeout => Program.Configuration?.GetValue<int>("Timeouts:Timeout") ?? 3000;
		//private Settings.SpecFlowConfiguration SpecFlowConfiguration => Program.SpecFlowConfiguration;

		#region Privates

		private int GetElementIndex(string position, IEnumerable<IWebElement> elements)
		{
			int? result = null;
			...
			if (!string.IsNullOrWhiteSpace(position))
			{
				...
				if (pos == "first") result = 0;
				else if (pos == "second") result = 1;
				else if (pos == "third") result = 2;
				else if (pos == "last") result = count - 1;
				else
				{
					int index;
			...
			if (result == null || result < 0) throw new IndexOutOfRangeException();
			return result.Value;
		}
		...
		private void TapElement(string name, string type, [CallerMemberName] string method = "")
		{
			...
			if (element == null) throw new ElementNotInteractableException($"Element [{name}] is not reachable");
			element.Click();
		}
		...
		private void TapElementAt(string name, string position, string type, [CallerMemberName] string method = "")
		{
			...
			if (elements == null || !elements.Any()) throw new NoSuchElementException($"Elements [{name}] are not reachable");
		...
		private void FillForm(Table table, string keyName = "field", string valueName = "value", [CallerMemberName] string method = "")
		{
			foreach (var row in table.Rows)
			{
				...
				var key = "";
				var value = "";
				if (row.ContainsKey(keyName)) key = row[keyName];
				if (row.ContainsKey(valueName)) value = row[valueName];
		...
		private bool KeyPress(string key, [CallerMemberName] string method = "")
		{
			var keyChar = "";
			switch (key.ToLower())
			{
		...
		//TODO: Move to another location?
		private bool SetText(string text, string type, [CallerMemberName] string method = "")
		{
			try
			{
		...
		public void UserEntersIntoTheCurrentlyActiveField(string text, string type)
		{
			try
			{
				SetText(text, type);
		...
		public void UserPressesOnTheKeyboard(string key)
		{
			try
			{
				KeyPress(key);
		...
		public void ThenWait(int time, string interval)
		{
		...
		public void WhenTheUserWaitsForSeconds(int numberOfSeconds)
		{
