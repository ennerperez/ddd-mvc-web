...
namespace Tests.Web.Services
{
  public class DefinitionService : IDefinitionService, IDisposable
  {
    ...
    internal IWebDriver Driver => Program.Driver;
    ...
    public KeyValuePair<SelectBy, string> GetStrategy(string selector, string token = "", string name = "")
    {
      ...
      {
        if (source.Ids.ContainsKey(key))
          result = new KeyValuePair<SelectBy, string>(SelectBy.Id, source.Ids[key]);
        else if (source.AutomationIds.ContainsKey(key))
          result = new KeyValuePair<SelectBy, string>(SelectBy.AccessibilityId, source.AutomationIds[key]);
        else if (source.AutomationNames.ContainsKey(key))
          result = new KeyValuePair<SelectBy, string>(SelectBy.Name, source.AutomationNames[key]);
        else if (source.AutomationCSSes.ContainsKey(key))
          result = new KeyValuePair<SelectBy, string>(SelectBy.CssSelector, source.AutomationCSSes[key]);
        else if (source.AutomationXPaths.ContainsKey(key))
          result = new KeyValuePair<SelectBy, string>(SelectBy.XPath, source.AutomationXPaths[key]);
        else if (source.AutomationClassNames.ContainsKey(key))
          result = new KeyValuePair<SelectBy, string>(SelectBy.ClassName, source.AutomationClassNames[key]);
        else if (source.AutomationLinkTexts.ContainsKey(key))
          result = new KeyValuePair<SelectBy, string>(SelectBy.LinkText, source.AutomationLinkTexts[key]);
        else if (source.AutomationPartialLinkTexts.ContainsKey(key))
          result = new KeyValuePair<SelectBy, string>(SelectBy.PartialLinkText, source.AutomationPartialLinkTexts[key]);
      }
    ...
    private T InvokeMethod<T>(KeyValuePair<SelectBy, string> location, [CallerMemberName] string method = "", params object[] values)
    {
      ...
      if (methodInfo == null) methodInfo = Driver.GetType().GetMethods().FirstOrDefault(m => m.Name == methodName);
      if (methodInfo == null) throw new NotImplementedException(methodName);
    ...
    public IWebElement FindElement(string selector, bool waitForElementToBeDisplayed = false)
    {
      ...
      if (isElementLoadAwaitable)
      {
        ...
        while (currentWait <= _definitionConfiguration.MaxSlowLoadingElementWaitTime)
        {
          ...
          try
          {
            ...
            if (waitForElementToBeDisplayed && element != null && element.Displayed) break;
          }
          catch (Exception)
          {
            element = null;
          ...
          else
          {
            ...
            if (stopWatch.ElapsedMilliseconds < _definitionConfiguration.ImplicitElementWaitTime)
            {
              ...
              if (extraWaitTimeNeeded > 0) Thread.Sleep(extraWaitTimeNeeded);
            }
      ...
      if (element == null && IsOptional(selector)) element = new WebElement((WebDriver)Driver, $"optional_{Guid.NewGuid().ToString()}");
    ...
    public IWebElement TryFindElement(string selector, int timeout = 1500, int maxAttempts = 5)
    {
      try
      {
        ...
        while (element == null)
        {
          ...
          if (element == null) Thread.Sleep(timeout);
      ...
      if (isElementLoadAwaitable)
      {
        ...
        while (currentWait <= _definitionConfiguration.MaxSlowLoadingElementWaitTime)
        {
          ...
          try
          {
            ...
            if (waitForElementToBeDisplayed && elements != null && elements.Any() && elements.All(m => m.Displayed)) break;
          }
          catch (Exception)
          {
            elements = null;
    ...
    public IReadOnlyCollection<IWebElement> TryFindElements(string selector, int timeout = 1500, int maxAttempts = 5)
    {
      try
      {
        ...
        while (elements == null)
        {
          ...
          if (elements == null) Thread.Sleep(timeout);
          if (maxAttempts > -1) attempts++;
        }
    ...
    public void SetCurrentPage(string name)
    {
      if (_automationContext.CurrentPage != name)
      {
        if (!string.IsNullOrWhiteSpace(name))
        {
          ...
          if (string.IsNullOrWhiteSpace(file)) throw new FileNotFoundException($"Definition file [{file}.ini] was not found");
