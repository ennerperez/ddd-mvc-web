...
namespace Tests.UITests.Services
{
    public class DefinitionService : IDefinitionService<IWebElement>, IDisposable
    {
        ...
        public DefinitionService(IAutomationContext automationContext, IDefinitionConfiguration definitionConfiguration)
        {
            ...
            {
                definitionsDirectory = _automationContext.ApplicationTarget.Split("-").Aggregate(definitionsDirectory, Path.Combine);
            }
            ...
            if (isElementLoadAwaitable)
            {
                ...
                while (currentWait <= _definitionConfiguration.MaxSlowLoadingElementWaitTime)
                {
                    ...
                    try
                    {
                        ...
                        if (waitForElementToBeDisplayed && element is { Displayed: true })
                        {
                            break;
                        }
                    }
        ...
        public IReadOnlyCollection<IWebElement> FindElements(string selector, bool waitForElementToBeDisplayed = false)
        {
            ...
            if (isElementLoadAwaitable)
            {
                ...
                while (currentWait <= _definitionConfiguration.MaxSlowLoadingElementWaitTime)
                {
                    ...
                    try
                    {
                        ...
                        if (waitForElementToBeDisplayed && elements != null && elements.Count != 0 && elements.All(m => m.Displayed))
                        {
                            break;
                        }
                    }
            ...
            if ((elements == null || elements.Count == 0) && IsOptional(selector))
            {
                elements = [new WebElement((WebDriver)Driver, $"optional_{Guid.NewGuid()}")];
            }
        ...
        public void SetCurrentPage(string name)
        {
            ...
            if (!string.IsNullOrWhiteSpace(name))
            {
                ...
                this.PageDefinitions = new Definition(name, "Page");
                config.Bind(this.PageDefinitions);
                this.PageDefinitions.PrepareAndValidate(_definitionConfiguration);
                ...
                var isValid = ValidateAllUniqueElements(this.PageDefinitions);
                if (!isValid)
                {
        ...
        {
            return this.Definitions.Select(item => item.OptionalElements.ContainsKey(name)).Any(containsKey => containsKey);
        }

        private bool ValidateAllUniqueElements(params Definition[] definitions)
        {
            var source = definitions.SelectMany(m => m.UniqueElements).ToArray();
            return source.Select(item => TryFindElement(item.Key)).All(element => element != null);
        }
...
#pragma warning restore CA1822
        {
            ...
            if (!string.IsNullOrWhiteSpace(_automationContext.ApplicationTarget))
            {
                definitionsDirectory = _automationContext.ApplicationTarget.Split("-").Aggregate(definitionsDirectory, (current, item) => Path.Combine(current, item));
            }
            ...
            {
                var value = item.Groups[1].Value.Replace(["&", "-", " ", "'"], "");
